...
//此时写入transactionid
		...
		$charityname=select("command", "schemeid_name");
		
			$type=select("command", "isdonate");
			
			$user= select("command", "statecode");
 $userscan=$user;
		 
		 
		 
		 $sql = "update ocl set transactionid='$transactionid' ORDER BY id DESC LIMIT 1 "; //標記結束transaction    //每次在載入首頁時候會檢查是否有0標記並上傳。
mysqli_query($link, $sql);
		 ...
		 if($_GET['charityID'])
		 {
			 $userscan=$_GET['charityID'];
		 }			 
		 
		 
		 setcookie('userscan',$userscan,time()+3600,"/" );
		 
		 	 
		 
		
		if($_GET['charityID'])   //对两个可选的选项，做重写。
		{
	  $userscan = $_GET['charityID'];
	 $charityname=$_GET['charityname'];
		$type=1;
	 	...
	 	$sql = "update command set userscan='$userscan',schemeid_name='$charityname'"; //开门
		mysqli_query($link, $sql);
 ...
 if(	$type=="1")
 {
	 ...
	 goto out;
 				...
 				echo	'<strong style=" border-radius:10px">'.selectword('Daily Rebate Limit：').'</strong> '; 
 		...
 		<tr>
 			...
 			<td width="0%" align="right"><span style="font-size:24px;"> <strong style=" border-radius:10px;margin-left:-20px"><?php if($type=="1") {echo 'Donation Amount：';}else{echo selectword('Rebate Amount：');}?> </strong> $<span id="msg1">0.0</span>
 	...
 	<span class="rebate" id="endbtn" style="bottom:-10px;left:50%;position:absolute;transform:translate(-50%,-50%)">   I'm finished</span>
 				...
 				$.ajax({
 					...
 					success: function(data, textStatus) {
 						if (document.getElementById("msg3").innerHTML == "0") {
 							document.getElementById("msg3").innerHTML = "Reached limit, recycling completed";
 						...
 						if (data.success == "1") {
 							...
 							datavalue = " "  ;
 							narn("success","Success","recycled successfully","");
...
playsound("sorry ineligible container");
 							evdata.data.btn.click();
 							/* document.getElementById("msg1").innerHTML=data.value;
 							document.getElementById("msg").innerHTML=data.num; */
 							datavalue = "Refused";
 							narn("error","Error","Sorry ineligible container","");
...
playsound("Bottle Shape And Barcode Does Not Match");
 ...
 playsound("Glass bottles not accepted");
 							evdata.data.btn.click();
 							/* document.getElementById("msg1").innerHTML=data.value;
 							document.getElementById("msg").innerHTML=data.num; */
 							datavalue = "Recycle failed";
 							 
							narn("error","Error","Glass bottles not accepted","");
