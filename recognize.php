...
die("<center>³ö´íÀ²333:2!</center>"); 
	...
	$sql="select *from barcode where barcode ='$barcode' and  omit='1' ";
	$result=mysqli_query($link,$sql);
	$result=mysqli_fetch_array($result);
...
$ch = curl_init();
 ...
 curl_setopt($ch, CURLOPT_URL, 'http://127.0.0.1:24401?threshold=0.0');
...
if(	$response['results'][$i]['x1']>0.35 && $response['results'][$i]['x1']<0.66)
...
if($confidence  > $zhixindu  &&  0.95<= $rate && $rate<=$ratevalue && $absweizhichazhi <=5 )
